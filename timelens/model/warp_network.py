...
def _pack_voxel_grid_for_flow_estimation(example):
    return th.cat([example["before"]["reversed_voxel_grid"], example["after"]["voxel_grid"]])


def _pack_images_for_warping(example):
    return th.cat([example["before"]["rgb_image_tensor"], example["after"]["rgb_image_tensor"]])
...
class Warp(nn.Module):
    ...
    def run_warp(self, example):
        ...
        warped, warped_invalid = warp.backwarp_2d(source=_pack_images_for_warping(example),
                                                  y_displacement=flow[:, 0, ...], x_displacement=flow[:, 1, ...])

        (before_flow, after_flow) = th.chunk(flow, chunks=2)
        (before_warped, after_warped) = th.chunk(warped, chunks=2)
        (before_warped_invalid, after_warped_invalid) = th.chunk(warped_invalid.detach(), chunks=2)
        return (
            before_warped,
            after_warped,
            before_flow,
