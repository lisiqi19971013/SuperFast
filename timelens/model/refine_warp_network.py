...
from timelens.model import fusion_network, warp_network
from timelens.model import unet


def _pack_for_residual_flow_computation(example):
    tensors = [example["middle"]["{}_warped".format(packet)] for packet in ["after", "before"]]
    tensors.append(example["middle"]["fusion"])
    return th.cat(tensors, dim=1)


def _pack_images_for_second_warping(example):
    return th.cat([example["middle"]["after_warped"], example["middle"]["before_warped"]])
...
class RefineWarp(warp_network.Warp, fusion_network.Fusion):
    ...
    def run_refine_warp(self, example):
        ...
        residual = self.flow_refinement_network(_pack_for_residual_flow_computation(example))

        (after_residual, before_residual) = th.chunk(residual, 2, dim=1)
        residual = th.cat([after_residual, before_residual], dim=0)

        refined, refined_invalid = warp.backwarp_2d(source=_pack_images_for_second_warping(example),
            y_displacement=residual[:, 0, ...], x_displacement=residual[:, 1, ...])
        
        (after_refined, before_refined) = th.chunk(refined, 2)
        (after_refined_invalid, before_refined_invalid) = th.chunk(refined_invalid.detach(), 2)
    ...
    def run_fast(self, example):
        warp_network.Warp.run_and_pack_to_example(self, example)  # warp的帧
        fusion_network.Fusion.run_and_pack_to_example(self, example)  # 合成帧

        residual = self.flow_refinement_network(_pack_for_residual_flow_computation(example))

        (after_residual, before_residual) = th.chunk(residual, 2, dim=1)
        residual = th.cat([after_residual, before_residual], dim=0)
        refined, _ = warp.backwarp_2d(source=_pack_images_for_second_warping(example),
                                      y_displacement=residual[:, 0, ...], x_displacement=residual[:, 1, ...])
