...
class EventSequence(object):
    ...
    def make_sequential_iterator(self, timestamps):
        ...
        for end_timestamp in timestamps[1:]:
            ...
            # print(start_index, end_index, start_timestamp, end_timestamp)
    ...
    def from_folder(cls, folder, image_height, image_width, event_file_template="{:06d}.npz"):
        ...
        import glob
        num = len(glob.glob(os.path.join(folder, event_file_template)))
        filenames = [os.path.join(folder, '%d'%f + event_file_template[-4:]) for f in range(num)]
        print(filenames)
    ...
    @classmethod
    def from_npy_files(cls, filename, image_height, image_width, start_time=None, end_time=None,):
        """Reads event sequence from numpy file list."""
        # features = load_events(filename)

        tmp = np.load(filename)
        tmp[:, -1] = tmp[:, -1] * 2 - 1
        features = tmp[:, [1, 2, 0, 3]]
